...
const AudioInfoContainer: FC<Props> = ({visible, closeHandler}) => {
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView>
        <View>
          <Text style={styles.title}>Tên: {onGoingAudio?.title}</Text>
          <View style={styles.ownerInfo}>
            <Text style={styles.title}>Người đăng: </Text>
            <AppLink title={onGoingAudio?.owner.name || ''} />
          </View>
          <Text style={styles.title}>Thể loại: {onGoingAudio?.category}</Text>
          <Text style={styles.about}>Thông tin: {onGoingAudio?.about}</Text>
        </View>
      </ScrollView>
    </View>
  );
